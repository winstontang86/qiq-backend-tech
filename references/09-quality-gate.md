...
| Q-7 | 组件清单与目标态架构 | 字段化表覆盖 §2.1 图中所有组件，含职责 / SLA / 上下游 / 部署形态；§2.1 给出**加上本需求后的整体架构图**（使用 `[新增]` / `[修改]` / `[复用]` 文本标记）；§2.4 给出**新增 / 修改 / 复用清单**与影响面摘要 | Blocker |
...
| Q-9a | 详细设计变更标注 | 已有系统演进时，§3 关键对象（存储 / 缓存 / MQ / DTO / 接口 / 流程 / 数据流图 / 状态机）均标注 `[新增]` / `[修改]` / `[复用]`，且与 §2.1 / §2.4 一致 | Blocker（已有系统演进时） |
...
| Q-32 | 工程一致性（已有工程） | §2.5 工程一致性表已声明并逐项判定（目录结构 / 命名 / 错误码 / 日志监控 / 基础组件 / 依赖发布 6 项）；偏离项已在 §4 立卡 | High（已有工程时） |
